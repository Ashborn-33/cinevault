...
interface AbandonedWatchingCardProps {
  item: LibraryItem
}

export function AbandonedWatchingCard({ item }: AbandonedWatchingCardProps) {
  ...
  const lastWatchedLabel = `Last watched ${diffDays} day${diffDays === 1 ? "" : "s"} ago`
  ...
  return (
    <div
      ...
      className="flex gap-4 p-4 border border-border/80 bg-surface/20 hover:bg-surface-hover/40 hover:border-border-hover rounded-card transition-all cursor-pointer shadow-sm group font-sans"
    >
      {/* Poster Cover */}
      ...
      <div className="flex-grow min-w-0 flex flex-col justify-between py-0.5 space-y-2">
        <div className="space-y-1">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="text-[9px] text-amber-500 font-bold shrink-0">{lastWatchedLabel}</span>
          </div>
        ...
        <div className="space-y-1">
          ...
          <div className="h-1.5 w-full bg-zinc-900 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-amber-500 to-amber-600 transition-all duration-standard"
      ...
      <div className="flex items-center shrink-0 pl-1 self-center">
        <Button
          size="icon"
          variant="secondary"
          className="h-8 w-8 rounded-full bg-zinc-900 border border-border/80 group-hover:bg-amber-500 group-hover:text-white group-hover:border-amber-500 transition-all shadow-sm"
          onClick={handleActionClick}
