...
export const DashboardService = {
  // getContinueWatching: Merges movies and TV shows sorted by last watched timestamp
  async getContinueWatching(userId: string): Promise<LibraryItem[]> {
    const [movies, tvShows] = await Promise.all([
      TrackingService.getContinueWatching(userId),
      TrackingService.getContinueWatchingShows(userId),
    ])
